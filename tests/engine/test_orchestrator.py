...
async def test_orchestrator_runs_simple_scenario(mock_device_agents):
        ...
        'steps': [
            {
                'device_id': 'CV_01',
                'action': 'send',
                'message': {'s': 1, 'f': 13, 'w_bit': True, 'body': ['A', 'TEST']}
            },
            {
                'delay_after_ms': 100
            },
            {
                'device_id': 'STK_01',
                'action': 'send',
                'message': {'s': 2, 'f': 1, 'w_bit': False, 'body': []}
            }
        ]
    }
    ...
    # Fixture 덕분에, load_device_agents_from_config 내부에서 DeviceAgent를 생성할 때
    # 우리가 만든 가짜 Agent들이 생성됩니다.
    orchestrator.load_device_configs('devices.json')
    orchestrator.run_scenario(scenario_data)

    # run_scenario는 백그라운드 태스크를 생성하므로, 완료될 때까지 잠시 기다려줍니다.
    await asyncio.sleep(0.2) # 시나리오의 delay(100ms)보다 긴 시간

    # 3. 검증
    cv_agent = mock_device_agents['CV_01']
    stk_agent = mock_device_agents['STK_01']

    # CV_01의 send_secs_message가 올바른 인자와 함께 호출되었는지 확인
    cv_agent.send_message.assert_awaited_once_with(
        s=1, f=13, w_bit=True, body_obj=['A', 'TEST']
    )

    # STK_01의 send_secs_message가 올바른 인자와 함께 호출되었는지 확인
    stk_agent.send_message.assert_awaited_once_with(
        s=2, f=1, w_bit=False, body_obj=[]
    )
